...
> **Pendiente de verificar (sellers de gestión asistida Taika/Bunker):** `Pedido.Logistica = "OCASATaika"` / `"AndreaniBunker"` (usado en `envio-despachado`, `recepcion-cambio`, `recepcion-devolucion`, `recepcion-garantia`, `etiqueta-devolucion`, `quiebre-stock`) se basa en los valores que muestra el panel de filtros del admin ("Pedidos por Depósito" → radio "Logística"), no en un JSON real de un pedido de esos depósitos. Se confirmó que el mail arma `Tienda`/`Pedido` leyendo la tabla `pedidos` (no de la consulta API de ejemplo que se revisó, cuyo esquema es `snake_case`: `id_deposito`, `lineas_pedido[].lp_logistica.logistica`, etc. — completamente distinto al `Tienda.*`/`Pedido.*` que usan las plantillas). Ninguno de los 3 JSON de ejemplo vistos (`id_deposito` 199, 45, 22) era un pedido real de Taika (`id_deposito` 198) o Bunker (`id_deposito` 197). **Falta un JSON real de un pedido con `id_deposito = 198` o `197` para confirmar que `lp_logistica.logistica` dice literalmente `"OCASATaika"`/`"AndreaniBunker"` y que se aplana a un único `Pedido.Logistica` a nivel de pedido** (asumible porque nunca hay pedidos con productos de más de un seller/depósito mezclados). Si al confirmar el valor real difiere, corregir los 6 templates PIM (los valores literales están todos en el bloque `{{ if (eq .Pedido.Logistica "OCASATaika") }}` / `"AndreaniBunker"` de cada uno) y los 10 escenarios de ejemplo en `examples/pim/shared/*-taika.json` / `*-bunker.json`. El lado VTEX (`order-invoiced`, campo `shippingData.logisticsInfo[0].deliveryIds[0].warehouseId`) ya se confirmó con JSON reales y no depende de esto.
